...
uint32_t _count_photos(const file_entry_t  **sorted_files,
...
void _png_memory_read(png_structp png_ptr, png_bytep data, png_size_t length) {
...
void _get_png_dimensions_from_memory(const unsigned char *data, size_t size,
...
void _get_jpeg_dimensions_from_memory(const unsigned char *data, size_t size,
                                      uint32_t *width, uint32_t *height,
                                      const verbose_mode_e *verbose_mode,
                                      const color_mode_e   *color_mode) {
...
bool _get_width_and_height(const verbose_mode_e *verbose_mode,
                           const color_mode_e *color_mode, uint32_t *width,
  ...
  if (is_png((unsigned char *)contents, st->size)) {
    ...
    _get_png_dimensions_from_memory((unsigned char *)contents, st->size, width,
                                    height, verbose_mode, color_mode);
    *ext = ".png";
    return true;
  }
  if (is_jpeg((unsigned char *)contents, st->size)) {
    ...
    _get_jpeg_dimensions_from_memory((unsigned char *)contents, st->size, width,
                                     height, verbose_mode, color_mode);
    *ext = ".jpg";
    return true;
  }
...
bool _handle_dimensions(const verbose_mode_e *verbose_mode,
                        const color_mode_e *color_mode, uint32_t *width,
                        uint32_t *height, const char *name,
                        unsigned char *contents, char **ext,
                        struct zip_stat *st) {
  if (!_get_width_and_height(verbose_mode, color_mode, width, height, ext,
                             contents, st)) {
...
void _handle_zip_entry(const file_entry_t  **sorted_files,
                       const verbose_mode_e *verbose_mode,
                       const color_mode_e   *color_mode,
                       const uint32_t *padding_length, struct zip *dest,
                       struct zip *z, uint32_t *photo_counter,
                       const uint32_t *i) {
  zip_uint64_t num_entries = zip_get_num_entries(z, 0);
  for (zip_uint64_t j = 0; j < num_entries; ++j) {
    const char *name     = zip_get_name(z, j, 0);
    ...
    unsigned char *contents = (unsigned char *)mallocv(
        "contents", *verbose_mode, *color_mode, st.size, -1);
    zip_fread(file, contents, st.size);

    printfv(*verbose_mode, *color_mode, "", "Handling file <%s>\n", name);

    uint32_t width = 0, height = 0;
    if (!_handle_dimensions(verbose_mode, color_mode, &width, &height, name,
                            contents, &ext, &st)) {
    ...
    printfv(*verbose_mode, *color_mode, "",
...
void extract_and_combine_cbz(const file_entry_t  **sorted_files,
  ...
  uint32_t total_photos =
      _count_photos(sorted_files, file_count, verbose_mode, color_mode);
  ...
  for (uint32_t i = 0; i < *file_count; ++i) {
    ...
    _handle_zip_entry(sorted_files, verbose_mode, color_mode,
                      (const uint32_t *)&padding_length, dest, z,
                      &photo_counter, &i);
  }
  zip_close(dest);
}
